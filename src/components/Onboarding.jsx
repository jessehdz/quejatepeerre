...
function Onboarding({ onDone }) {
    ...
    return (
        <BottomSheet
            onClose={() => {}}
            zIndex={isLast ? 900 : 1500}
            className={`ob-sheet${isLast ? ' ob-sheet-final' : ''}`}
        >
            {/* Close — always available, exits onboarding entirely */}
            <CloseButton variant="plain" className="ob-close" onClick={handleSkip} ariaLabel="Cerrar" />
            ...
            {!isLast && (
                <CtaButton className="ob-cta" label={current.cta} onClick={handleCta} />
            )}
